...
class TestGetClient:
    async def test_returns_client_from_valid_context(
        self, mock_context: MagicMock, mock_client: AsyncMock
    ) -> None:
        client = await get_client(mock_context)
        assert client is mock_client

    async def test_raises_tool_error_when_request_context_is_none(self) -> None:
    ...
    async def test_raises_tool_error_when_lifespan_context_is_none(self) -> None:
        ctx = MagicMock(spec=Context)
